...
type Recipe={id:string;title:string;kcal:number|null;category:string;vegetarian:boolean}
export default function Home(){
 ...
 useEffect(()=>{(async()=>{const a=await ensureAccount(); if(!a){setLoading(false);return} const sb=supabaseBrowser(); setName(a.profile?.display_name||'Clément'); setGoal(a.profile?.target_weight_kg??null); const w=await sb.from('weight_entries').select('weight_kg').eq('user_id',a.user.id).order('measured_on',{ascending:false}).limit(1).maybeSingle(); setWeight(w.data?.weight_kg??null)
 const today=new Date().toISOString().slice(0,10), week=mondayOf(); const {data:mw}=await sb.from('menu_weeks').select('id').eq('household_id',a.householdId).eq('week_start',week).maybeSingle(); if(mw){const {data:items}=await sb.from('menu_items').select('menu_date,slot,servings,is_free_meal,recipes(id,title,kcal)').eq('week_id',mw.id).eq('menu_date',today); setMenu(items||[])} setLoading(false)})()},[])
 ...
 <section className="card"><div className="row"><h2>🍽️ Menu du jour</h2><Link href="/planning">Voir</Link></div>{menu.length?menu.map((m:any)=><div className="meal" key={m.slot}><div className="thumb">{m.slot==='petit_dejeuner'?'☀️':m.slot==='dejeuner'?'🍱':'🌙'}</div><div><div className="muted">{labels[m.slot]}</div><strong>{m.is_free_meal?'Repas libre':m.recipes?.title}</strong><div className="kcal">{m.recipes?.kcal?`${Math.round(m.recipes.kcal)} kcal / portion`:''}</div></div></div>):<div className="empty"><strong>Ton menu n’est pas encore généré.</strong><p className="muted">Va dans Planning pour créer ta semaine automatiquement.</p><Link className="btn" href="/planning">Créer mon planning</Link></div>}</section>
